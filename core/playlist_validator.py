...
class PlaylistValidator:
    ...
    def validate_all(self, auto_fix: bool = True) -> ValidationReport:
        ...
        for pl_file in self.playlists_dir.glob("*.json"):
            ...
            if result.duplicates_removed or result.titles_cleaned or result.playable_synced:
...
if __name__ == "__main__":
    ...
    for r in report.results:
        if r.duplicates_removed or r.titles_cleaned or r.playable_synced or r.errors:
            print(f"  {r.playlist_name}: dupes={r.duplicates_removed}, titles={r.titles_cleaned}, synced={r.playable_synced}")
