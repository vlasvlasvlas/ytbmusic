...
class SkinLoader:
    ...
    def __init__(self):
        ...
        self.zones = (
            {}
        )  # For template mode: {"title": {"line": 10, "col": 5, "width": 35}, ...}
    ...
    def load(self, skin_path: str) -> Tuple[Dict, List[str]]:
        ...
        if self.mode not in ["freestyle", "template"]:
            raise ValueError(
                f"Invalid mode '{self.mode}'. Must be 'freestyle' or 'template'"
            )
    ...
    def _fit_to_canvas(
        self, lines: List[str], width: int = CANVAS_WIDTH, height: int = CANVAS_HEIGHT
    ) -> List[str]:
        """
    ...
    def _render_freestyle(
        self, lines: List[str], context: Dict[str, str], pad_width: int, pad_height: int
    ) -> List[str]:
        """
    ...
    def _render_template(
        ...
        for zone_name, zone_config in self.zones.items():
            if isinstance(zone_config, dict) and "line" in zone_config:
                ...
                self._render_zone(
                    rendered, zone_name, zone_config, context, pad_width, pad_height
                )
            else:
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if len(line) > CANVAS_WIDTH:
                    errors.append(
                        f"Line {i+1} too long: {len(line)} chars (max {CANVAS_WIDTH})"
                    )
    ...
    def create_error_skin(self, errors: List[str]) -> List[str]:
        ...
        lines.append(
            "║  The selected skin could not be loaded. Please fix the following issues:   ║"
        )
        lines.append(
            "║                                                                            ║"
        )
        ...
        if len(errors) > 15:
            lines.append(
                f"║  ... and {len(errors)-15} more errors.                                     ║"
            )
        ...
        lines.append(
            "║  Press 'S' to switch skins, or edit the file to fix it.                    ║"
        )
        lines.append(
            "╚════════════════════════════════════════════════════════════════════════════╝"
        )
